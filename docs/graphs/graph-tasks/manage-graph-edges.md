...
1. Click **Graphs**.
2. Click the graph that you want to create a new edge for.
3. Click an empty space on the canvas, then click the plus sign that appears.
4. (Optional) In the **Create node** window, enter the vertex **_key**. If you do not enter a key, then Macrometa automatically generates one for you.
5. In the **Create node** window, select the **Collection** to add the vertex to.
6. Click **Create**.
