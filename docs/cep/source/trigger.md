...
Triggers allow events to be periodically generated. A trigger also works like a [stream](stream.md) with a predefined schema. You can use a trigger as a source of events for a stream worker in place of `CREATE SOURCE` or `CREATE STREAM`.
