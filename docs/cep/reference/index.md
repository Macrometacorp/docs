...
| Data Transformations | Support `data extraction` and `reconstruction of messages` using inline mathematical and logical operations,  inbuilt [functions](./functions) and custom functions in `JavaScript` for processing `JSON`, `string`, `time`, `math`, `regex`, and others.|
