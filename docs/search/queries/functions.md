...
Search functions can be used in a [SEARCH operation](queries/operations.md) to form an Search expression to filter a View. The functions control the Search functionality without having a returnable value in C8QL.
...
- returns nothing: the function can only be called in a [SEARCH operation](queries/operations.md) and throws an error otherwise
