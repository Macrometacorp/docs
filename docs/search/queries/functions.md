...
Search functions can be used in a [SEARCH operation](operations.md) to form an Search expression to filter a View. The functions control the Search functionality without having a returnable value in C8QL.
...
Scoring functions return a ranking value for the documents found by a [SEARCH operation](operations.md). The better the documents match the search expression the higher the returned number.
