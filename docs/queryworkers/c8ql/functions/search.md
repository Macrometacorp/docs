...
Search functions can be used in a [SEARCH operation](../operations/search) to form an Search expression to filter a View. The functions control the Search functionality without having a returnable value in C8QL.
...
- **analyzer** (string, _optional_): name of an [Analyzer](../../../search/analyzers). Uses the Analyzer of a wrapping `ANALYZER()` call if not specified or defaults to `"identity"`
- returns nothing: the function can only be called in a [SEARCH operation](../operations/search) and throws an error otherwise
...
- **analyzer** (string): name of an [Analyzer](../../../search/analyzers).
