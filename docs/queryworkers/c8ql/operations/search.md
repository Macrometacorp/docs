...
The `SEARCH` keyword starts the language construct to filter Views of type Search. Conceptually, a View is just another document data source, similar to an array or a document/edge collection, over which you can iterate using a [FOR operation](for) in C8QL:
...
See [Search Views](search) on how to set up a View.
...
`expression` must be an Search expression. The full power of Search is harnessed and exposed via special [Search functions](../functions/search), during both the search and sort stages. On top of that, common C8QL operators are supported:
...
You can use the special `includeAllFields` [View property](search) to index all (sub-)fields of the source documents if desired.

## Arrays and trackListPositions

Array elements are indexed individually and can be searched for as if the attribute had each single value at the same time. They behave like a _disjunctive superposition_ of their values as long as the [**trackListPositions**](search) View setting is `false` (default).
...
This is different to `FILTER` operations, where you would use an [array comparison operator](../operators#array-comparison-operators) to find an element in the array:
...
String tokens (see [Analyzers](search)) are also indexed individually, but not all Analyzer types return multiple tokens. If the Analyzer does, then comparison tests are done per token/word. For example, given the field `text` is analyzed with `"text_en"` and contains the string `"a quick brown fox jumps over the lazy dog"`, the following expression will be true:
...
The documents emitted from a View can be sorted by attribute values with the standard [SORT() operation](sort), using one or multiple attributes, in ascending or descending order (or a mix thereof).
...
If the (left-most) fields and their sorting directions match up with the [primary sort order](search) definition of the View then the `SORT` operation is optimized away.

Apart from simple sorting, it is possible to sort the matched View documents by relevance score (or a combination of score and attribute values if desired). The document search via the `SEARCH` keyword and the sorting via the [Search Scoring Functions](../functions/search#scoring-functions), namely `BM25()` and `TFIDF()`, are closely intertwined.
...
The [BOOST() function](../functions/search#boost) can be used to fine-tune the resulting ranking by weighing sub-expressions in `SEARCH` differently.
