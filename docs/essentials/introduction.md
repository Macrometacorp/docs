...
* **Collections** are a grouping of JSON documents and are like tables in a RDBMS. You can create any number of collections in a geo GeoFabric. And a collection can have any number of documents. A collection can be a `kv` or `document` collection.
...
* **Search** is a full-text search engine for information retrieval on one or more linked collections. Also referred to as a `View`.
...
* **Stream Processors** perform complex event processing in real-time on streams.

For more information, refer to the [GeoFabrics](geofabrics.md) section.
...
Macrometa GDN is schemaless, meaning you do not need to define valid attributes for documents. Every single document can have a completely different structure and still be stored together with other documents in a single collection. In practice, there will be common denominators among the documents in a collection, but the GDN itself doesn't force you to limit yourself to a certain data structure.
 ...
 * **Document Collections** - Also referred to as vertex collections in the context of graphs. These collections can be created as local or global collections
    ...
    * A `global` collection replicates its data while also maintaining state and consistency across all regions in your GeoFabric. 
...
Each document has a unique `primary key` which identifies it within its collection. Furthermore, each document is uniquely identified by its `document handle` across all collections in the same GeoFabric. Different revisions of the same document (identified by its handle) can be distinguished by their `document revision`. Any transaction only ever sees a single revision of a document. 
...
In GDN, the `_rev` strings are in fact timestamps. They use the local clock of the DBserver that actually writes the document and have millisecond accuracy. Actually, a "Hybrid Logical Clock" is used (for this concept see [this paper](http://www.cse.buffalo.edu/tech-reports/2014-04.pdf)).
...
Different servers in a cluster might have a clock skew, so between different shards or collections the timestamps are not guaranteed to be comparable. The Hybrid Logical Clock addresses this issue by verifying that messages sent between nodes are timestamped on both nodes, and that the second node is a later time than the first. 

If there is *causality* between events on different servers, timestamps increase from cause to effect. Thus, sometimes a server must take timestamps that seem to come from the future of its own clock. However, it still produces increasing timestamps. If the clock skew is short, then your timestamps will somewhat reflect the time when the document revision was actually written.
...
!!! note
    The `_rev` attribute can be used as a precondition for queries to avoid lost update situations. For example, if a client fetches a document from the server, modifies it locally (but with the _rev attribute untouched), and sends it back to the server to update the document, but meanwhile the document was changed by another operation, then the operation is cancelled by the server. Otherwise, the client would inadvertently overwrite changes made to the document.

To find a particular revision of a document, you need the document revision and the document handle or key.
...
In SQL databases, you have the notion of a relation table to store `n:m` relationships between two data tables. An `edge collection` is similar to these relation tables. A `vertex collection` resemble the data tables with the objects to connect.
...
GDN is a real-time materialized view engine. Streams & stream processing are intregral part of GDN. Stream processing feature provides users geo-replicated stream data processing capabilities to integrate streaming data and takes action based on streaming data.
...
Stream processing can be used for:
...
* Integrating streaming data and perform actions based on data streams. The action can be a single request to a service or a complex enterprise integration flow.
