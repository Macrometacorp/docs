...
:::note
If you intend to use [joins](../../queries/query-examples/joins.md) it may be clever to use non-sparsity and maybe even uniqueness for that attribute, else all items containing the `null` value will match against each other and thus produce large results.
:::
